...
// Mandi price data (in production, fetch from data.gov.in or Agmarknet API)
const MANDI_PRICES = [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Filter prices
    let filteredPrices = MANDI_PRICES;
    if (crop) {
      filteredPrices = filteredPrices.filter(p => p.crop_name === crop);
    }
    if (state) {
    ...
    const crops = [...new Set(MANDI_PRICES.map(p => p.crop_name))];
    const states = [...new Set(MANDI_PRICES.map(p => p.state))];

    // Get trend for selected crop
    let trend = null;
    if (crop) {
      trend = CROP_TRENDS.find(t => t.crop_name === crop);
    }
